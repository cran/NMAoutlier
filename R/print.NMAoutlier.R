print.NMAoutlier <- function(x, digits = 4) {
  ...
  cat("Study entered into the basic set in each step of forward search algorithm:\n")
  prmatrix(x$basic)
  ##
  ##
  cat("\n")
  cat("Monitored measures of basic set in each step of forward search algorithm:\n")
  cat("Heterogeneity and inconsistency measures:\n")
  ##
  ##
  cat("\n")
  cat("Q statistics and heterogeneity:\n")
  prmatrix(cbind(formatN(x$Qb), formatN(x$Qhb), formatN(x$Qib), formatN(x$taub, digits)),
